...
class TestMerchantAtEntry:
    """Choosing the merchant when the row is written, instead of merging later.

    This is the whole replacement for the merge queue. Every way a second
    spelling could get in — typing one, importing one, editing one — has to
    land on the name already in use, because nothing downstream will fix it.
    """

    def _make(self, client, category_id, description, **overrides):
        return client.post(
            "/api/transactions",
            json={
                "occurred_on": "2026-03-14",
    ...
    def test_a_blank_merchant_is_guessed_from_the_description(
        self, client, category_id
    ):
        body = self._make(client, category_id, "HARRIS TEETER #999 CHARLOTTE NC")
        assert body["merchant_key"] == "Harris Teeter"

    def test_a_typed_name_snaps_to_the_spelling_already_in_use(
        self, client, category_id
    ):
        """The near-duplicate the merge queue used to exist to clean up."""
        body = self._make(
            client, category_id, "anything at all", merchant_key="harris teeter"
        )
        assert body["merchant_key"] == "Harris Teeter", (
            "a differently-cased name must join the existing merchant, "
            "not become a second one"
        )

    def test_an_explicit_name_beats_the_guess(self, client, category_id):
        body = self._make(
            client, category_id, "BILT CARD HOUSING", merchant_key="ZZ My Landlord"
        )
        assert body["merchant_key"] == "ZZ My Landlord"

    def test_a_description_with_no_merchant_in_it_gets_none(self, client, category_id):
        """A bare reference number names nobody, and a blank says so.

        'NON-CHASE ATM WITHDRAW 690124' deliberately is *not* this case — the
        normalizer keeps it whole because the digits are too short to read as
        an id. That is a known miss, and the field being editable is what
        covers it now that no merge queue does.
        """
        body = self._make(client, category_id, "1234567890")
        assert body["merchant_key"] is None

    def test_editing_snaps_too_and_blank_clears(self, client, category_id):
        txn = self._make(client, category_id, "SOMETHING ELSE")
        r = client.patch(
            f"/api/transactions/{txn['id']}", json={"merchant_key": "HARRIS TEETER"}
        )
        assert r.json()["merchant_key"] == "Harris Teeter"

        r = client.patch(f"/api/transactions/{txn['id']}", json={"merchant_key": ""})
        assert r.json()["merchant_key"] is None, (
            "a row with no payee must be able to say so"
        )

    def test_a_descriptor_suffix_snaps_to_the_shorter_existing_name(
        self, client, category_id
    ):
        """The regression the dropped pattern table would otherwise have caused.

        The old schema stored a pattern per descriptor, so a hand-merge taught
        it that NETFLIX.COM was Netflix. Without the patterns the guess is
        'Netflix Com' — a second name for the same subscription, and no queue
        left to fold it back in.
        """
        body = self._make(client, category_id, "NETFLIX.COM")
        assert body["merchant_key"] == "Netflix"

    def test_the_longest_prefix_wins_so_uber_eats_is_not_uber(
        self, client, category_id
    ):
        """'Uber' and 'Uber Eats' are different places and both exist here.

        Taking any prefix would file every delivery under Uber. Taking the
        longest keeps them apart, which is the difference between a useful
        default and a silent merge.
        """
        body = self._make(client, category_id, "UBER EATS DELIVERY 4471")
        assert body["merchant_key"] == "Uber Eats"

    def test_the_picker_ranks_by_use_not_alphabetically(self, client):
        """278 of 403 names are used once; alphabetical buries the real ones."""
        keys = client.get("/api/merchants/keys?limit=5").json()
        counts = [k["count"] for k in keys]
        assert counts == sorted(counts, reverse=True)
        assert keys[0]["key"] == "Harris Teeter"

    def test_the_picker_searches(self, client):
        keys = client.get("/api/merchants/keys?q=harris").json()
        assert keys and all("harris" in k["key"].lower() for k in keys)

    def test_a_corrected_merchant_on_import_is_what_gets_written(
        self, client, category_id
    ):
        """The preview's guess is editable, and the edit is what lands.

        Without this the import is still authoritative and a wrong guess is
        only fixable by merging — which no longer exists.
        """
        csv = "Date,Description,Amount\n2029-04-01,ZZ WEIRD DESCRIPTOR 88,4.00\n"
        preview = client.post("/api/imports/preview", data={"text": csv}).json()
        row = preview["rows"][0]
        assert row["merchant_key"] == "Zz Weird Descriptor 88"
        ...
        written = next(
            ...
            if t["raw_description"] == "ZZ WEIRD DESCRIPTOR 88"
        )
        assert written["merchant_key"] == "Harris Teeter"
...
class TestCsvImport:
    ...
    def test_preview_infers_categories_from_imported_history(self, client):
        ...
        assert harris["merchant_key"], "expected the merchant to be suggested"
...
class TestIdenticalRowsInOneFile:
    ...
    def test_a_new_merchant_repeated_lands_under_one_name(self, client, category_id):
        """The same crash one level down: the second row could not see the
        merchant the first had just created, and made a second one.

        Now a merchant is a string on the row, so there is no insert to race —
        but the two rows must still agree on one spelling, which is what the
        picker guarantees and what this checks.
        """
        csv = (
            "Date,Description,Amount\n"
        ...
        keys = client.get("/api/merchants/keys?q=zzqq novel").json()
        assert len(keys) == 1, "one shop, one name"
...
class TestCategoryOptions:
    ...
    def test_the_suggestion_is_history_not_a_stored_default(self, client, session):
        """There is nowhere left to store a default, which is the point.

        merchants.default_category_id recorded whichever transaction created
        the merchant and never moved; it disagreed with the merchant's own
        history for twelve of them. The suggestion has to be the most-used
        category read at the time it is asked for.
        """
        ...
        top = max(row["category_options"], key=lambda o: o["count"])
        actual = (
            session.query(Transaction)
            .filter(Transaction.merchant_key == row["merchant_key"])
            .filter(Transaction.category_id == top["id"])
            .count()
        )
        assert actual == top["count"], "the count must come from the transactions"
...
class TestReconciliation:
    ...
    def test_unmatched_rows_offer_candidates(self, client, category_id):
        ...
        proving nothing.
        """
        keys = client.get("/api/merchants/keys?limit=1").json()
        assert keys, "expected merchant names in the database"
        name = keys[0]["key"]
    ...
    def test_linking_a_merchant_makes_the_row_reconcile(self, client, category_id):
        ...
        """
        merchant = client.get("/api/merchants/keys?limit=1").json()[0]
        created = client.post(
            "/api/fixed-costs",
            json={
                "description": f"ZZ {merchant['key']} Membership",
                "amount": "5.00",
                "category_id": category_id,
                "effective_from": "2020-01-01",
            },
        ...
        client.patch(
            ...
            json={"merchant_key": target["suggestions"][0]},
        )
...
class TestRecurringBackfill:
    ...
    def test_an_explicit_merchant_link_is_honoured(self, session, category_id):
        ...
        merchant = "ZZ Test Landlord Card"
